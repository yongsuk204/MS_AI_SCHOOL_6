...
def qlist():
    ...
    if kw:
        query = query.filter(Question.subject.ilike(f'%{kw}%'))

    
    question_list = query.order_by(Question.create_date.desc()).paginate(page=page, per_page=5)

    return render_template(
        'question/question_list.html',
        question_list=question_list,  # <- paginate() 결과
        page=page,
        kw=kw,
    )
